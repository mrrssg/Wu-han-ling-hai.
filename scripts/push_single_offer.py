...
def build_of24_payload_with_discount(of22: dict, new_origin: float,
    ...
    The discount MUST be a flat `discount.price` (the `discount-price` column
    in the Excel template), NOT `discount.ranges`. Mirakl import error report
    3664-2085655 confirmed: sending discount.ranges leaves discount.price null
    and Mirakl rejects with "The discount price is incorrect: must not be
    null". `ranges` is only for tiered (quantity-threshold > 1) discounts.

    If the live offer has no discount window to reuse, fall back to an
    origin-price-only payload (build_of24_payload_from_of22 already stripped
    the empty discount).
    """
    ...
    has_discount = (
        existing.get("discount_price") is not None
        or bool(existing.get("ranges"))
    )
    if not has_discount:
        return payload

    discount = {"price": round(float(new_discount), 2)}
