...
4. 保持原文案的风格和长度量级（标题仍是标题的长度，五点仍是一条bullet，不超过原文长度的1.2倍）
5. reason 用中文说明为什么这么改
硬红线（违反任何一条即不合格）：
- 全文严禁出现任何品牌名/商标词（Costway、VEVOR、我方店铺名、竞品品牌都不行）——即使供应商资料里有品牌也必须剥掉
- 标题不带颜色后缀，标题末尾不准挂光秃秃的数字（承重等数字要带 Capacity/Holds/Supports 语义包装）
- 若原五点是"Title Case副标题+冒号"格式，改后必须保持同样格式（不是ALL CAPS）
- 供应商资料里没有依据的数字（座位数/承重/尺寸）一律不写，宁缺勿编
只输出JSON：
