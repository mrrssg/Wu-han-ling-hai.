...
class ExportService:
    ...
    def export_haoya_unshipped_xlsx() -> Tuple[BytesIO, str]:
        ...
        # 2) 获取豪雅价格表（按你逻辑 newestdropship）
        haoya_price_map: Dict = DBManager.fetch_sku_map('newestdropship')

        # 3) 过滤：sku 在价格表里 => 豪雅
        haoya_orders: List[Dict] = []
        for o in all_orders:
            sku = (o.get('sku') or '').strip()
            if sku and sku in haoya_price_map:
                haoya_orders.append(o)

        # 4) 生成 xlsx
        bio = ExportService.build_haoya_xlsx(haoya_orders, haoya_price_map)
    ...
    def export_sishun_unshipped_xlsx() -> Tuple[BytesIO, str]:
        ...
        # 2) 获取豪雅价格表（按你逻辑 newestdropship）
        sishun_price_map: Dict = DBManager.fetch_sku_map('newestdropship_vevor')

        # 3) 过滤：sku 在价格表里 => 豪雅
        sishun_orders: List[Dict] = []
        for o in all_orders:
            sku = (o.get('sku') or '').strip()
            if sku and sku in sishun_price_map:
                sishun_orders.append(o)

        # 4) 生成 xlsx
        bio = ExportService.build_sishun_xlsx(sishun_orders, sishun_price_map)
    ...
    def export_dajian_unshipped_xls() -> Tuple[BytesIO, str]:
        ...
        # 2) 大建价格表（用于筛选 SKU）
        dajian_price_map: Dict = DBManager.fetch_sku_map('newestdropship_dajian')

        # 3) 过滤出大建订单（Costway_SKU 在大建表中）
        dajian_orders: List[Dict] = []
        for o in all_orders:
            sku = (o.get('sku') or '').strip()
            if sku and sku in dajian_price_map:
                dajian_orders.append(o)

        # 4) 生成 xls
        bio = ExportService.build_dajian_xls(dajian_orders)
